...
if (!isActionAccessible($guid, $connection2, '/modules/Professional Development/requests_editApprover.php')) {
    ...
    $professionalDevelopmentRequestApproversID = $_POST['professionalDevelopmentRequestApproversID'] ?? '';
    ...
    if (empty($professionalDevelopmentRequestApproversID) || !$requestApproversGateway->exists($professionalDevelopmentRequestApproversID)) {
        ...
        if ($requestApproversGateway->update($professionalDevelopmentRequestApproversID, ['finalApprover' => $finalApprover])) {
            $URL .= '&return=success0';
        } else {
            $URL .= '&return=error2';
        }
